...
in this [note][]).
* When overriding `logstash.yml`, `http.host: XXX.XXX.XXX.XXX` should always be included
to make default probes work. If restricting HTTP API to XXX.XXX.XXX.XXX is required by
using `http.host: XXX.XXX.XXX.XXX`, default probes should be disabled or overridden
